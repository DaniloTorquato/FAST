...
def gather_time(results_path):
    ...
    time_path = os.path.join(results_path, "time_starts", "Atualization_time_starts.txt")
    result[str_sel_time+"atualization_starts"] = read_time(time_path)

    time_path = os.path.join(results_path, "time", str_fast_pw+"_time.txt")
    result[str_prio_p_time] = read_time(time_path)/num_iterations
    
    time_path = os.path.join(results_path, "time_starts", str_fast_pw+"_time_starts.txt")
    result[str_prio_p_time+"_starts"] = read_time(time_path)/num_iterations
    ...
    time_path = os.path.join(results_path, "time_starts", str_fastazi_p+"_time_starts.txt")
    result[str_comb_time+"_starts"] = read_time(time_path)/num_iterations
    ...
    result["DPTT_ekstazi_time_starts"] = read_time(time_path)/30
    
    

    result[str_ekstazi_time] = result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]
    result["STARTS_time"] = result[str_sel_time]
    
    result[str_fast_time] = result[str_fast_prep_time] + result[str_prio_p_time]
    ...
    result["str_fastazi_p_time_test"] = max((result[str_build_time] + result[str_sel_time]), (result[str_fast_prep_time] + result[str_prio_p_time])) + max(result[str_comb_time], result[str_test_time])
    result["str_fastazi_s_time_test"] = max((result[str_build_time] + result[str_sel_time]), result[str_fast_prep_time]) + max(result[str_prio_s_time], result[str_test_time])
    
    result[str_fast_time+"_starts"] = max((result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]), (result[str_fast_prep_time+"_starts"] + result[str_prio_p_time+"_starts"]))
    result[str_fastazi_p_time+"_starts"] = max((result[str_sel_time+"_starts"]), (result[str_fast_prep_time+"_starts"] + result[str_prio_p_time+"_starts"])) + result[str_comb_time+"_starts"]
    result[str_fastazi_s_time+"_starts"] = max((result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]), result[str_fast_prep_time+"_starts"]) + result[str_prio_s_time+"_starts"]
    result["str_fastazi_p_time_test_starts"] = max((result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]), (result[str_fast_prep_time+"_starts"] + result[str_prio_p_time+"_starts"])) + max(result[str_comb_time+"_starts"], result[str_test_time+"_starts"])
    result["str_fastazi_s_time_test_starts"] = max((result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]), result[str_fast_prep_time+"_starts"]) + max(result[str_prio_s_time+"_starts"], result[str_test_time+"_starts"])
    
    
    result["FAST_T"] = (result[str_fast_prep_time] + result[str_prio_p_time])
    result["FAST_T_starts"] = (result[str_fast_prep_time+"_starts"] + result[str_prio_p_time+"_starts"])
    result["DPTT_t"] = result[str_build_time] + result[str_test_time] + result["DPTT_time"]
    result["DPTT_ekstazi_t"] = result[str_build_time] + result[str_sel_time] + result[str_test_time] + result["DPTT_ekstazi_time"]
    result["DPTT_ekstazi_t_starts"] = result[str_build_time] + result[str_test_time] + result[str_sel_time+"_starts"] + result["DPTT_ekstazi_time_starts"]
    
    result["Build  + selection"] = result[str_build_time] + result[str_sel_time]
    result["Build  + selection + test"] = result[str_build_time] + result[str_sel_time] + result[str_test_time]
    
    result["Build  + selection_starts"] = result[str_build_time+"_starts"] + result[str_sel_time+"_starts"]
    result["Build  + selection + test_starts"] = result[str_build_time+"_starts"] + result[str_sel_time+"_starts"] + result[str_test_time+"_starts"]
...
def time_avg(time):
    suites = [str_build_time, str_test_time, str_sel_time, 
              ...
              str_fastazi_p_time, str_fastazi_s_time,"DPTT_time","DPTT_ekstazi_time","FAST_T", "DPTT_t", "DPTT_ekstazi_t" ,"str_fastazi_p_time_test","str_fastazi_s_time_test","DPTT_ekstazi_t_starts", "Build  + selection", "Build  + selection + test", str_sel_time+"_starts", str_prio_p_time+"_starts", str_prio_s_time+"_starts", str_comb_time+"_starts", str_fast_prep_time+"_starts", str_build_time+"_starts", str_test_time+"_starts", "DPTT_ekstazi_time_starts", "STARTS_time", str_fast_time+"_starts", str_fastazi_p_time+"_starts", str_fastazi_s_time+"_starts", "str_fastazi_p_time_test_starts", "str_fastazi_s_time_test_starts", "FAST_T_starts", "Build  + selection_starts", "Build  + selection + test_starts", str_sel_time+"atualization_starts"]
    avg = {}
    s = {} # sum
    c = {} # count
